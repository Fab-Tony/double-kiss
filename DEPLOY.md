...
- **The cloud sandbox cannot reach `api.github.com`** (Anthropic proxy blocks it) — the REST API and `gh` CLI don't work here. Use **git over HTTPS**; that path works. Clone anywhere convenient (this repo is canonical; auth from `~/lifework/secrets/double-kiss-pat.env`).
